...
在 `3.2 用户体验旅程图` 中，必须用 Mermaid 或画板画出用户体验旅程图。正式飞书交付优先使用画板。默认结构是：顶部一排是旅程阶段和“发生”主线，发生节点从左到右串联；每个发生节点下方依次挂“对应核心场景”“痛点”“期望”。

用户体验旅程图必须满足以下要求：

- 每个旅程阶段必须拆成四个独立格子：发生事项、对应核心场景、用户痛点、期望解决方式。
- 发生节点必须横向串联，形成一条完整主线，直到场景结束。
- 对应核心场景、痛点和期望只能作为对应发生节点下方的子节点，禁止进入横向主线。
...
- Mermaid 只能近似固定布局。若目标是飞书正式交付，优先把 Mermaid 转成画板 / SVG，以保证框宽、间距、对齐和视觉效果稳定。检查线上 PRD 时，必须同时查看画板预览或原始节点，不能只看文档里的 `<whiteboard>` 占位。
...
flowchart TB
  subgraph MAIN["发生"]
    direction LR
    A["1. 阶段名称<br/>发生：具体用户事项"]
    B["2. 阶段名称<br/>发生：具体用户事项"]
    C["3. 阶段名称<br/>发生：具体用户事项"]
    A --> B --> C
  end

  subgraph SCENE["对应核心场景"]
    direction LR
    A_s["对应核心场景：场景名称"]
    B_s["对应核心场景：场景名称"]
    C_s["对应核心场景：场景名称"]
...
- `3.2` 有四层用户体验旅程图，且“对应核心场景”能映射到 `3.3 核心场景表`。
...
- 密集需求信息优先用表格表达。
- 核心场景中的用户体验旅程图必须使用 Mermaid 或画板表达；正式交付优先以画板承载。
